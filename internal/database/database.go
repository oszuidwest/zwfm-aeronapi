...
// DB is an interface that matches the subset of sqlx.DB methods we use.
...
func CountItems(ctx context.Context, db DB, schema string, table types.Table, hasImage bool) (int, error) {
	...
	qt, err := types.QualifiedTable(schema, table)
	if err != nil {
...
func UpdateEntityImage(ctx context.Context, db DB, schema string, table types.Table, id string, imageData []byte) error {
	...
	entityType := types.EntityTypeForTable(table)
	...
	if err != nil {
		return &types.DatabaseError{Operation: fmt.Sprintf("bijwerken van %s", entityType), Err: err}
	}
	return nil
}
...
func DeleteEntityImage(ctx context.Context, db DB, schema string, table types.Table, id string) error {
	...
	if err != nil {
		return &types.DatabaseError{Operation: fmt.Sprintf("verwijderen van %safbeelding", entityType), Err: err}
	}
	...
	if rowsAffected == 0 {
		return types.NewNotFoundError(entityType+"afbeelding", id)
	}
...
func GetEntityImage(ctx context.Context, db DB, schema string, table types.Table, id string) ([]byte, error) {
	...
	if err == sql.ErrNoRows {
		return nil, types.NewNotFoundError(entityType, id)
	}
	if err != nil {
		return nil, &types.DatabaseError{Operation: fmt.Sprintf("ophalen %s afbeelding", entityType), Err: err}
	}
	if imageData == nil {
		return nil, types.NewNoImageError(entityType, id)
	}
